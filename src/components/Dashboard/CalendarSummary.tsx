...
const CalendarSummary = ({
  ...
  const start =
    Temporal.PlainDate.compare(monthStart, today) > 0 ? monthStart : today;
  const upcoming = getUpcomingTransactions(
    user.transactions,
    start,
    7,
    3
  ...
  return (
    ...
    >
      ...
      <div
        className="calendar-summary-stats"
        aria-label={`${monthName} totals`}
      >
        ...
        <div className="calendar-summary-stat calendar-summary-net">
          ...
          <strong
            className={monthTotal.balance >= 0 ? "is-positive" : "is-negative"}
          >
            {monthTotal.balance >= 0 ? "+" : "-"}$
            {formatCurrency(Math.abs(monthTotal.balance))}
          </strong>
        </div>
      </div>
